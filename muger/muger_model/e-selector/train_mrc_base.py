...
class DataManager:
    def __init__(self):
        ...
        self.index = 0
       # dev_fnm = './mrc_data/all_neglink_data.json'
       # train_fnm = './mrc_data/mrcdata_train.json'
       
       # dev_qids, dev_pairs, dev_labels, dev_spans = self.load_dat_new(dev_fnm)
       # train_qids, train_pairs, train_labels, train_spans = self.load_dat_new(train_fnm)
       # ipdb.set_trace()
       # self.dev_samples = self.trans_samples(dev_qids, dev_pairs, dev_labels, dev_spans)
       # self.train_samples = self.trans_samples(train_qids, train_pairs, train_labels, train_spans)
       # ipdb.set_trace()
       # fp = open('mrc_data/samplesmrc_train_neg.json','w')
       # fp.write(json.dumps(self.dev_samples))
       # fp.close()
       # fp = open('mrc_data/samplesmrc_train.json','w')
       # fp.write(json.dumps(self.train_samples))
       # fp.close()
        ...
        with open('./mrc_data/samplesmrc_train_neg.json') as f:
            neg_train_samples = json.loads(f.read())
...
class ModelDefine(nn.Module):
    def __init__(self):
        ...
        self.sfc = nn.Linear(768,1)
        self.efc = nn.Linear(768,1)
...
class Model:
    ...
    def validate(self, which="dev", epoch=-1):
       ...
       # dev_questions = {}  
        ...
        #ipdb.set_trace()  
        if which == "dev":
            self.save("trained_models/mrc_base_check_point_{}.pt".format(epoch), epoch)
